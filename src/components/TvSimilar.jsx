...
import useRecommendedfetchTv from '../utils/Hooks/useRecommendedfetchTv';

const TvSimilar = ({ tvId }) => {
  const dispatch = useDispatch();
  const similar = useSelector((state) => state.tvdetails.Similar) || [];
  const recommended = useSelector((state) => state.tvdetails.Recommended) || [];
  const navigate = useNavigate();

  useSimilarTvfetch(tvId);
  useRecommendedfetchTv(tvId);
